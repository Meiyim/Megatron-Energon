...
def _pool_worker(task_queue, result_queue, produce_fn):
    """Standalone worker function for spawn-safe AggregatorPool."""
    while True:
        task = task_queue.get()
        if task is None:
            break
        for item in produce_fn(task):
...
class AggregatorPool(Generic[T_input_data, T_aggregation_data, T_result]):
    ...
    def __init__(
        ...
        self._final_result_data_queue = _ctx.Queue()
    ...
    def process(self) -> T_result:
        ...
        # Must use spawn: fork inherits patched S3/EPath connection state, causing EPath downloads to hang.
        multiprocessing.set_start_method("spawn", force=True)
        try:
            for w_id in range(self.num_workers):
                p = multiprocessing.Process(
                    target=_pool_worker,
                    args=(self.task_queue, self.result_queue, self.user_produce_data),
                    daemon=True,
                )
                p.start()
                workers.append(p)
