...
def _patch_s3_fork_safety():
    ...
    Only forked children build a rust client; the main process stays on boto3. Its
    checkpoint uploads are made safe by _patched_upload, which re-runs _upload_file's
    f.seek(0) on every retry so a throttled retry re-uploads the full object instead
    of resuming from EOF and writing 0 bytes.
    """
